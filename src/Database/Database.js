            ...
            SQLite.echoTest().then(() => {
                SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {
                    ...
                    db.executeSql('SELECT 1 FROM Imovel LIMIT 1').then(() => {
                        ...
                        db.transaction((tx) => {
                            tx.executeSql('CREATE TABLE IF NOT EXISTS Imovel (id INTEGER PRIMARY KEY AUTOINCREMENT, endereco VARCHAR(30), finalidade VARCHAR(30), tipo VARCHAR (30), valor INTEGER, imagem TEXT);');
                        }).then(() => {
                ...
                db.transaction((tx) => {
                    ...
                    tx.executeSql('SELECT * FROM Imovel', []).then(([tx, results]) => {
                        ...
                        for (let i = 0; i < len; i++) {
                            ...
                            lista.push({ id, endereco, finalidade, tipo,valor, imagem  });
                        }
                        console.log(lista);
                        resolve(lista);
                    });
